...
argument_list|()
operator|+
literal|"/home/images/language_48.png"
argument_list|)
expr_stmt|;
name|defaultThumbnails
operator|.
...
return|;
block|}
comment|/**      * Returns the Language Annotations      * @since 0.11.0      * @return      * @throws ParseException      */
specifier|public
name|Collection
argument_list|<
name|EntityExtractionSummary
argument_list|>
name|getLanguageOccurrences
parameter_list|()
throws|throws
name|ParseException
block|{
...
argument_list|()
decl_stmt|;
comment|//we need to process those to show multiple mentions
comment|//            if (graph.filter(textAnnotation, DC_RELATION, null).hasNext()) {
...
argument_list|)
decl_stmt|;
comment|//TextAnnotations without fise:selected-text are no longer ignored
comment|//            if(text == null){
...
expr_stmt|;
block|}
comment|//in case of a language annotation use the detected language as label
if|if
condition|(
name|DC_LINGUISTIC_SYSTEM
operator|.
...
condition|)
block|{
comment|//new extraction summary
name|occurrenceMap
operator|.
name|put
argument_list|(
...
block|{
comment|//extraction summary with this text and suggestions already
comment|//present ... only add a mention to the existing
name|existingSummary
operator|.
name|addMention
argument_list|(
...
block|}
block|}
comment|/**      * Mentions of {@link EntityExtractionSummary EntityExtractionSummaries}.       * @author Rupert Westenthaler      *      */
specifier|public
specifier|static
class|class
name|Mention
...
literal|"[global]"
expr_stmt|;
comment|//throw new IllegalStateException("The name for a Mention MUST NOT be NULL!");
block|}
elseif|else
if|if
condition|(
...
literal|0
return|;
comment|//assume as equals if name and suggestionSet is the same
block|}
else|else
block|{
comment|//sort by mention
if|if
condition|(
name|start
operator|!=
...
name|o
decl_stmt|;
comment|//if name and suggestions are the same ... consider as equals
if|if
condition|(
name|getName
argument_list|()
...
return|;
block|}
comment|//return !(name != null ? !name.equals(that.name) : that.name != null);
block|}
annotation|@
name|Override
specifier|public
