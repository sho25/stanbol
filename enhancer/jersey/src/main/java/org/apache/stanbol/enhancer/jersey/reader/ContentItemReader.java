...
name|uriInfo
decl_stmt|;
comment|/**      * Used to access the Content-Language header for STANBOL-660      */
annotation|@
name|Context
name|HttpHeaders
name|headers
decl_stmt|;
comment|/**      * Clerezza LiteralFactory      */
specifier|private
name|LiteralFactory
name|lf
init|=
...
argument_list|)
expr_stmt|;
comment|//STANBOL-660: set the language of the content if explicitly parsed in the request
name|String
...
condition|)
block|{
comment|//language codes are case insensitive ... so we convert to lower case
name|contentLanguage
operator|=
name|contentLanguage
operator|.
...
argument_list|)
expr_stmt|;
name|createParsedLanguageAnnotation
argument_list|(
name|contentItem
argument_list|,
name|contentLanguage
argument_list|)
expr_stmt|;
comment|// previously only the dc:language property was set to the contentItem. However this
comment|// information is only used as fallback if no Language annotation is present. However
comment|// if a user explicitly parses the language he expects this language to be used
comment|// so this was change with STANBOL-1417
comment|//            EnhancementEngineHelper.set(contentItem.getMetadata(), contentItem.getUri(),
comment|//                DC_LANGUAGE, new PlainLiteralImpl(contentLanguage));
block|}
return|return
name|contentItem
return|;
block|}
comment|/**      * Creates a fise:TextAnnotation for the explicitly parsed Content-Language      * header. The confidence of this annotation is set<code>1.0</code> (see       *<a href="https://issues.apache.org/jira/browse/STANBOL-1417">STANBOL-1417</a>).      * @param ci the {@link ContentItem} to the the language annotation      * @param lang the parsed language      */
specifier|private
name|void
name|createParsedLanguageAnnotation
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|//add the fise:Enhancement information
name|m
operator|.
name|add
argument_list|(
...
name|createTypedLiteral
argument_list|(
literal|"Content-Language Header of the request"
argument_list|)
argument_list|)
argument_list|)
expr_stmt|;
comment|//add fise:TextAnnotation information as expected by a Language annotation.
name|m
operator|.
name|add
argument_list|(
...
return|;
block|}
comment|/**      * Getter for the<code>Content-Language</code> header      * @return the language of the content as parsed in the request or       *<code>null</code> if the header is not present.      */
specifier|private
name|String
name|getContentLanguage
parameter_list|()
...
name|warn
argument_list|(
literal|" ... only single valued Content-Language headers are "
operator|+
literal|"supported (will use first language of {})"
argument_list|,
name|languages
argument_list|)
expr_stmt|;
