...
begin_comment
comment|/**  * Default implementation of the Rick.  * @author Rupert Westenthaler  *  */
end_comment
...
expr_stmt|;
block|}
comment|//First check the RICK ID and
name|log
operator|.
name|info
argument_list|(
...
name|ID
argument_list|,
literal|"The Rick Configuration does not define a ID for the Rick"
argument_list|)
throw|;
block|}
else|else
...
name|NAME
argument_list|,
literal|"The Rick Configuration does not define a name for the Rick"
argument_list|)
throw|;
block|}
else|else
...
name|PREFIX
argument_list|,
literal|"The Rick Configuration does not define a Prefix for the Rick"
argument_list|)
throw|;
block|}
try|try
...
name|PREFIX
argument_list|,
literal|"The Prefix configured for the RICK is not an valied URI (prefix="
operator|+
name|config
operator|.
name|getRickPrefix
...
throw|;
block|}
comment|//next get the reference to the configured RickYard
name|log
operator|.
name|info
argument_list|(
...
name|format
argument_list|(
literal|"Unable to set Filter %s to ServiceListener for RickYard! -> add ServiceListener without Filter"
argument_list|,
name|rickYardFilterString
argument_list|)
argument_list|,
...
comment|// -> we need to do that after the init of the Rick-yard, because than we
...
name|msg
init|=
literal|"Unable to create Symbol based on the representation as stored in the Rick"
decl_stmt|;
name|log
operator|.
name|warn
...
argument_list|()
operator|+
literal|" is currently not active in the RICK -> return null"
argument_list|)
expr_stmt|;
return|return
literal|null
...
argument_list|)
expr_stmt|;
comment|//create a FieldMapper containing the Rick Mappings and the Site specific mappings
...
argument_list|)
decl_stmt|;
comment|//Store the symbol and the mappedEntity in the rickYard
name|rickYard
operator|.
name|store
argument_list|(
