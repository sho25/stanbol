...
name|context
decl_stmt|;
comment|/**      * The directory used to keep the IDs of registered resources      */
name|File
name|configDir
decl_stmt|;
...
name|IllegalStateException
argument_list|(
literal|"Unable to create the config directory '"
operator|+
name|configDir
operator|.
name|getAbsolutePath
...
block|}
block|}
comment|//start with the assumption that the framework is active
name|this
operator|.
name|context
operator|.
...
expr_stmt|;
block|}
comment|/**      * Checks if the state is not {@link BundleEvent#STOPPED},       * {@link BundleEvent#STOPPING} or {@link BundleEvent#UNINSTALLED}      * @return the state      */
specifier|private
name|boolean
name|isFrameworkActive
parameter_list|()
block|{
return|return
operator|(
...
name|debug
argument_list|(
literal|"bundleChanged(bundle:{}|state:{})"
argument_list|,
name|event
operator|.
name|getBundle
...
argument_list|()
decl_stmt|;
comment|//if(event instanceof Framework)
switch|switch
condition|(
name|event
operator|.
...
name|debug
argument_list|(
literal|"register request for Bundle {}"
argument_list|,
name|bundle
operator|.
name|getSymbolicName
...
name|debug
argument_list|(
literal|"ignore because Framework is shutting down!"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|IllegalStateException
argument_list|(
literal|"Unablt to save the IDs of"
operator|+
literal|"the resources installed for bundle '"
operator|+
name|bundle
operator|.
name|getSymbolicName
...
block|}
block|}
comment|/**      * Used to stores/overrides the ids of installed resource for a bundle.      * @param bundle      * @param resources      * @throws IOException      */
specifier|private
name|void
name|storeConfig
parameter_list|(
...
block|}
block|}
comment|/**      * Reads the installed resources for the parsed bundle and deletes the      * configuration afterwards.      * @param bundle the bundle      * @return the list of resources      * @throws IOException if the config file was not found or on any other      * exception while reading the file      */
specifier|private
name|Collection
argument_list|<
name|String
...
name|namespace
decl_stmt|;
comment|//do not search the path within the file name!!
name|int
name|nsIndex
init|=
name|Math
...
name|debug
argument_list|(
literal|"unregister request for Bundle {}"
argument_list|,
name|bundle
operator|.
name|getSymbolicName
...
name|info
argument_list|(
literal|" ... remove configurations for Bundle {}"
argument_list|,
name|bundle
operator|.
name|getSymbolicName
...
name|warn
argument_list|(
literal|"Unable to remove installed Resources for Bundle '"
operator|+
name|bundle
operator|.
name|getSymbolicName
argument_list|()
operator|+
literal|"' because an Exeption while"
operator|+
literal|"reading the installed ID from the config file"
argument_list|,
name|e
argument_list|)
expr_stmt|;
