...
begin_comment
comment|/**  * Historical. Just kept to be able to lookup the original code while debugging  * the new {@link ManagedSolrServerImpl} and {@link StandaloneManagedSolrServer}  * implementations.<p>  * Hopefully I do never commit this class again. If you find it feel free to  * create a JIRA issue for its removal!  *   * @author Rupert Westenthaler  * @deprecated replaced by {@link ManagedSolrServerImpl} when running within an  * OSGI environment and the {@link StandaloneManagedSolrServer} if outside  * of an OSGI environment  */
end_comment
...
begin_comment
comment|//    @Property(name = ManagedSolrServer.MANAGED_SOLR_DIR_PROPERTY,
end_comment

begin_comment
comment|//        value = ManagedSolrServer.DEFAULT_SOLR_DATA_DIR)
end_comment
...
block|{
comment|//implements ManagedSolrServer {
comment|/**      * The logger      */
specifier|private
specifier|final
name|Logger
...
decl_stmt|;
comment|/**      * The internally managed Solr server      */
specifier|private
name|SolrServerAdapter
name|managedSolrServer
decl_stmt|;
...
comment|/**      * The path to the directory used to store properties of uninitialised referenced sites.      *<p>      * Such sites are one that are created by using {@link #createSolrIndex(String, String, Map)} but the      * {@link DataFileProvider} does not yet provide the necessary data to initialise the index.      *<p>      * This directory will store properties files with the indexName as name, properties as extension and the      * properties as value      */
specifier|private
specifier|final
name|String
...
block|}
block|}
comment|/**      * Returns the directory used to store the configurations of uninitialised Solr Indexes      *       * @return the directory      */
specifier|private
name|File
name|getUninitialisedSiteDirectory
parameter_list|(
...
literal|true
expr_stmt|;
comment|//lookup/init the managed SolrDir
name|File
name|managedDir
init|=
name|lookupManagedSolrDir
...
name|info
argument_list|(
literal|"... activate {} with Directory {}"
argument_list|,
name|this
operator|.
name|getClass
...
control|)
block|{
comment|/*              * NOTE: Managed SolrCores may be already registered in the solr.xml.              * In that case they are initialised when creating the ManagedSolrServer.              * If not we need to add them manually. The following if(..) prevents              * that cores are registered (and therefore initialised) twice!.              */
if|if
condition|(
operator|!
name|managedSolrServer
