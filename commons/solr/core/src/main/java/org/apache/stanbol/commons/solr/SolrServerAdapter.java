...
name|info
argument_list|(
literal|"   ... create OSGI enabled SolrCore (conf: {}"
argument_list|,
name|solrCof
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|//if succeeded (re-)register the swapped core
name|registerCoreService
argument_list|(
name|core1
argument_list|,
...
parameter_list|)
block|{
comment|//STANBOL-1235: we want to unregister the old before registering the new
comment|//   but we do not want all solrCores to be closed as otherwise the
comment|//   SolrCore would be deactivated/activated. So if we find a old
comment|//   registration we will acquire a 2nd reference to the same core
comment|//   for the time of the re-registration
name|SolrCore
name|sameCore
init|=
literal|null
...
argument_list|)
expr_stmt|;
comment|//2nd reference to the core
name|log
operator|.
name|info
argument_list|(
...
condition|)
block|{
comment|//clean up the 2nd reference
name|sameCore
operator|.
name|close
argument_list|()
...
expr_stmt|;
block|}
comment|//TODO: solrCores is private ...
comment|//solrCores.addCreated(created); // For persisting newly-created cores.
return|return
name|created
return|;
block|}
comment|/*          * Create from local configuration (replaces the method with the same          * name in the parent class)          */
specifier|private
name|SolrCore
name|createFromLocal
parameter_list|(
...
return|;
block|}
comment|/*          * Create from Zookeeper (replaces the method with the same name in          * {@link ZkContainer})          */
specifier|private
name|SolrCore
name|createFromZk
parameter_list|(
...
argument_list|()
argument_list|,
comment|// TODO: Core properties are not accessible -> parse null
comment|// ConfigSolrXml.getCoreProperties(instanceDir, dcore),
literal|null
argument_list|,
name|zkController
argument_list|)
