...
argument_list|)
block|,
comment|/**      * Verb related POS types for Spanish language.      * I was not able to find the list, so POS tag results where used to       * create this configuration       */
name|ES_VERB
argument_list|(
literal|"es"
argument_list|,
...
argument_list|)
block|,
comment|/**      * POS types one needs typically to follow to build {@link Chunk}s over       * Nouns (e.g. "University_NN of_IN Otago_NNP" or "Geneva_NNP ,_, Ohio_NNP").      * I was not able to find the list, so POS tag results where used to       * create this configuration.<p>      * For now "SP" and all "F*" tokens referring to '.', ';', ...      */
name|ES_FOLLOW
argument_list|(
literal|"es"
argument_list|,
