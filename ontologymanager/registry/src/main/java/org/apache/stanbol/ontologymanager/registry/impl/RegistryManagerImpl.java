...
name|model
operator|.
name|OWLOntologyLoaderConfiguration
import|;
end_import
...
specifier|final
name|boolean
name|_RETAIN_INCOMPLETE_DEFAULT
init|=
literal|true
decl_stmt|;
specifier|private
...
name|offline
decl_stmt|;
comment|/*      * Maps libraries (values) to ontologies (keys). This does not keep track of the loading status of each      * library, as it is handled by the library itelf.      */
specifier|private
name|Map
argument_list|<
name|IRI
...
argument_list|()
decl_stmt|;
comment|// If we are retaining incomplete registries, do not throw exceptions if imports fail.
name|conf
operator|.
...
block|{
comment|// Relative IRI : use data file provider
name|log
operator|.
name|debug
argument_list|(
literal|"Found relative IRI {} . Will try to retrieve from data file providers."
argument_list|,
name|iri
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Failed to obtain OWL ontology from resource {}"
argument_list|,
name|loc
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|/*                  * Works like object property assertions, in case hasOntology and isOntologyOf are not                  * detected to be object properties (e.g. due to a failure to load codolight or the registry                  * metamodel).                  */
name|OWLAnnotationProperty
name|prop
init|=
name|axiom
...
condition|)
block|{
comment|// Get usages of hasOntology as an object property
for|for
control|(
name|OWLIndividual
name|value
...
argument_list|)
expr_stmt|;
comment|// Get usages of hasOntology as an annotation property
for|for
control|(
name|OWLAnnotationAssertionAxiom
name|ann
...
argument_list|()
decl_stmt|;
comment|// If some populate*() method has created it, it will be there.
name|RegistryItem
name|ront
init|=
name|population
...
argument_list|)
decl_stmt|;
comment|// Otherwise populating it will also put it in population.
if|if
condition|(
name|ront
operator|==
...
condition|)
block|{
comment|// Get usages of isOntologyOf as an object property
for|for
control|(
name|OWLIndividual
name|value
...
argument_list|)
expr_stmt|;
comment|// Get usages of isOntologyOf as an annotation property
for|for
control|(
name|OWLAnnotationAssertionAxiom
name|ann
