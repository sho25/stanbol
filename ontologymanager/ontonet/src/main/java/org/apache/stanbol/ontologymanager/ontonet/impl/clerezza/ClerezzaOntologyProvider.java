...
comment|// TODO Profile this method. Are we getting rid of rdfData after adding its triples?
name|OWLOntologyID
name|publicKey
init|=
name|OWLUtils
...
condition|)
block|{
comment|// keymap.setMapping(physical, uriref);
name|keymap
operator|.
name|addAlias
argument_list|(
...
argument_list|)
throw|;
comment|/*          * The operation will have to: (1) refuse to comply if there is at least one handle; (2) clear all          * aliases; (3) delete the graph;          */
block|}
annotation|@
name|Override
specifier|public
...
name|KILL
case|:
comment|// In the current implementation it is not GC'ed yet, but it might be in the future
if|if
condition|(
name|s
operator|!=
...
control|)
block|{
comment|// Set<OntologyCollector> handles = descriptor.getHandles(publicKey);
comment|// if (handles.isEmpty()) throw new IllegalStateException(
comment|// "No handles on an ontology apparently managed by session " + s);
comment|// else if (handles.size() == 1) {
comment|// OntologyCollector handle = handles.iterator().next();
comment|// if (handle != s) throw new IllegalStateException(
comment|// "Unexpected single handle differs from " + s);
comment|// }
comment|// removeOntology(publicKey);
block|}
break|break;
default|default:
break|break;
