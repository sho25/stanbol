...
parameter_list|()
block|{
comment|// Set<String> result = new HashSet<String>();
comment|// for (UriRef u : store.listTripleCollections())
comment|// result.add(u.getUnicodeString());
...
parameter_list|)
block|{
comment|// TODO Instead of copying the code, reuse it.
name|long
name|before
init|=
name|System
...
decl_stmt|;
comment|//
comment|// IRI location = null;
comment|// if (force) location = null;
comment|// else for (OWLOntologyIRIMapper mapper : mappers) {
comment|// location = mapper.getDocumentIRI(ontologyIri);
comment|// if (location != null) break;
comment|// }
comment|// if (location == null) {
comment|// if (isOfflineMode()) throw new IllegalStateException(
comment|// "Cannot retrieve " + ontologyIri + " while Stanbol is in offline mode. "
comment|// + "No resource with that identifier was found locally.");
comment|// else location = ontologyIri;
comment|// }
comment|//
comment|// log.info("found {} in {}", ontologyIri, location);
name|boolean
name|loaded
init|=
literal|false
...
try|try
block|{
comment|// final URLConnection con = location.toURI().toURL().openConnection();
comment|// con.setRequestProperty("Accept", format);
comment|// final InputStream is = con.getInputStream();
if|if
condition|(
name|data
operator|!=
...
condition|)
block|{
comment|// System.out.println("I am mapping "+ontologyIri+" to key "+s);
name|ontologyIdsToKeys
operator|.
name|put
argument_list|(
...
name|debug
argument_list|(
literal|"Load and Store completed in {} ms"
argument_list|,
operator|(
name|System
operator|.
...
name|iri
expr_stmt|;
comment|// Was most likely a SimpleMGraph
comment|//        if (rdfData instanceof MGraph) {
comment|//            graph = (MGraph) rdfData;
comment|//        } else
block|{
name|UriRef
name|uriref
