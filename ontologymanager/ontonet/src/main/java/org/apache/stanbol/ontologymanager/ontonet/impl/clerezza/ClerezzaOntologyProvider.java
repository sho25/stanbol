...
condition|)
block|{
comment|// FIXME rewrite import statements so that only the scope name needs to be added!
...
argument_list|)
decl_stmt|;
comment|// // More concise / efficient / brutal implementation, but messes up ontology name.
comment|// TripleCollection tempGraph = new SimpleMGraph();
comment|// for (UriRef ref : revImps)
comment|// if (!loaded.contains(ref)) {
comment|// tempGraph.addAll(store.getTriples(ref));
comment|// loaded.add(ref);
comment|// }
comment|// OWLOntology merged = OWLAPIToClerezzaConverter.clerezzaGraphToOWLOntology(tempGraph, mgr);
return|return
name|merged
return|;
block|}
