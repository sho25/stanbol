...
comment|/*          * rdfData should be a SimpleGraph, so we shouldn't have a problem creating one with the TcProvider          * and adding triples there, so that the SimpleGraph is garbage-collected.          *           * TODO this occupies twice as much space, which should not be necessary if the provider is the same          * as the one used by the input source.          */
name|UriRef
name|uriref
init|=
operator|new
