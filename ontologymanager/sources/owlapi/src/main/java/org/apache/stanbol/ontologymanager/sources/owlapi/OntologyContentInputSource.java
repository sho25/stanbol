...
begin_comment
comment|/**  * An input source that tries to parse an in-memory {@link OWLOntology} object from an input stream. This  * ontology input source will try to resolve imports and fail if one cannot be resolved. To setup a custom  * configuration, such as adding IRI mappers, the constructor  * {@link #OntologyContentInputSource(InputStream, OWLOntologyManager)} can be used with a configured  * {@link OWLOntologyManager}.  *   * @author alexdma  *   */
end_comment
