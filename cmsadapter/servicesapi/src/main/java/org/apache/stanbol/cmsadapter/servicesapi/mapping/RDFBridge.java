...
begin_comment
comment|/**  * This interface declares the methods to create mappings between RDF data and JCR/CMIS based repositories.  * Each {@code RDFBridge} indicates a set of target resource in the RDF data. Resources filtered based on the  * configuration of the bridge are created as nodes/object in the repository. This interface also aims to get  * configuration values for properties and names of objects to be created in the repository.  *   * @author suat  *   */
end_comment
...
name|RDFBridge
block|{
comment|/**      * Gets the target CMS path of this bridge. Existing CMS objects to be updated will be searched under this      * path and new ones will be created considering this as root.      *       * @return target CMS path      */
name|String
