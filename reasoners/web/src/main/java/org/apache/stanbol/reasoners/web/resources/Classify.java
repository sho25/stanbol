...
block|}
block|}
comment|/**      * To trasform a sequence of rules to a Jena Model 	 *  	 * @param owl 	 *            {OWLOntology object contains a single recipe}      * @return {An Set<SWRLRule> that contains the SWRL rule.}      */
specifier|private
name|Set
argument_list|<
name|SWRLRule
...
argument_list|)
expr_stmt|;
comment|// We escape back the XML entities
comment|//kReSRules = kReSRules.replaceAll("&lt;", "<");
comment|//kReSRules = kReSRules.replaceAll("&gt;", ">");
name|System
operator|.
name|out
...
argument_list|)
expr_stmt|;
comment|/*  if(true)             return Response.status(Status.OK).build();*/
try|try
block|{
if|if
condition|(
