...
begin_comment
comment|/**  *   * @author elvio, andrea.nuzzolese, alberto.musetti  */
end_comment
...
return|;
block|}
comment|/*             AddRecipe instance = new AddRecipe(ruleStore);              // String ID =             // kresRuleStore.getOntology().getOntologyID().toString().replace(">","").replace("<","")+"#";              boolean ok = instance.addSimpleRecipe(IRI.create(recipe), description);              if (!ok) {              	return Response.status(Status.CONFLICT).build();              } else {                 ruleStore.saveOntology();                 return Response.status(Status.OK).build();             }             */
block|}
catch|catch
parameter_list|(
name|Exception
...
name|error
argument_list|(
literal|"Error while adding a rule to a recipe."
argument_list|,
name|e
argument_list|)
expr_stmt|;
