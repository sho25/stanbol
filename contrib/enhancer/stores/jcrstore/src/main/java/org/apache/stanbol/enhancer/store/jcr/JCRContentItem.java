...
block|}
comment|//TODO:
comment|// 1) handle BNodes (create Nodes and use WEAKREFERENCE in triples?)
comment|// 2) handle typed literals (use JCR PropertyTypes?)
comment|//    Maybe we need an own LiteralFactory
comment|// 3) handle Plain literals (use String and additional lang property?)
name|Node
name|tripleNode
init|=
name|graphNode
...
name|IllegalStateException
argument_list|(
literal|"Unable to parse RDF data from Node '"
operator|+
name|jcrNodePath
argument_list|,
name|e
