...
begin_comment
comment|/**  * Enhancement Engine that provides the ability to assign a text document to a set of concepts indexed in a  * dedicated Solr core. The assignment logic comes from terms frequencies match of the text of the document to  * categorize with the text indexed for each concept.  *   * The data model of the concept tree follows the SKOS model: concepts are organized in a hierarchical  * "scheme" with a "broader" relation (and the inferred "narrower" inverse relation). Concepts can also  * optionally be grounded in the real world by the mean of a foaf:primaryTopic link to an external resource  * such as a DBpedia entry.  *   * A document is typically classified with the concept by using the dct:subject property to link the document  * (subject) to the concept (object).  *   * The Solr server is expected to be configured with the MoreLikeThisHandler and the matching fields from the  * engine configuration.  *   * This text classifier method sometimes goes by the name of "Rocchio classification" or "Nearest Centroid  * classification" in the IR and machine learning literature. It is often slightly less accurate than fitting  * penalized linear model such as linear kernel Support Vector Machines or penalized Logistic Regression but  * has the advantage to scale to large number of categories (e.g. more that tens of thousands) without having  * to load the full statistical model in memory thanks to the use of the inverted index datastructure that  * also provides the feature extraction and TF-IDF weighting for free.  *   * Furthermore it could be refined by using a "Learning to Rank" approach by training a RankSVM, Gradient  * Boosted Trees or Random Forests on the output on the raw output of the Rocchio classifier so as to re-rank  * candidate classes more finely. The Learning to Rank refinement is not implemented yet.  */
end_comment
...
name|DEFAULT_SOLR_CORE_CONFIG
init|=
literal|"default-topic-model.solrindex.zip"
decl_stmt|;
specifier|public
specifier|static
specifier|final
...
literal|"metadata"
decl_stmt|;
comment|/**      * The reference to the SolrCore used ny the TopicClassificationEngine.      * The default is the engine name with the suffix '-model'. This also supports      * the {server-name}:{index-name} syntax. if n      */
specifier|public
specifier|static
specifier|final
name|String
...
decl_stmt|;
comment|/**      * The name of the Solr Index archive (default: "default-topic-model.solrindex.zip").      * The file is loaded by using the DataFileProvider infrastructure. The archive may      * also include a pre-trained model.      */
specifier|public
specifier|static
specifier|final
name|String
...
specifier|final
name|String
name|DEFAULT_SIMILARTITY_FIELD
init|=
...
init|=
literal|"org.apache.stanbol.enhancer.engine.topic.conceptUriField"
decl_stmt|;
specifier|public
specifier|static
specifier|final
name|String
name|DEFAULT_CONCEPT_URI_FIELD
init|=
literal|"concept"
decl_stmt|;
specifier|public
...
specifier|final
name|String
name|DEFAULT_PRIMARY_TOPIC_URI_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_MODEL_UPDATE_DATE_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_MODEL_EVALUATION_DATE_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_MODEL_ENTRY_ID_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_FALSE_POSITIVES_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_FALSE_NEGATIVES_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_POSITIVE_SUPPORT_FIELD
init|=
...
specifier|final
name|String
name|DEFAULT_NEGATIVE_SUPPORT_FIELD
init|=
...
name|TRAINING_SET_ID
init|=
literal|"org.apache.stanbol.enhancer.engine.topic.trainingSetId"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
argument_list|)
decl_stmt|;
comment|/**      * The "text/plain" mime type      */
specifier|public
specifier|static
specifier|final
name|String
...
comment|// estimate of the number of topics occurring in each documents
specifier|protected
name|String
name|engineName
decl_stmt|;
...
specifier|protected
name|ManagedSolrServer
name|managedSolrServerDummy
decl_stmt|;
comment|// trick to call the super class binders
comment|/**      * Only used for testing outside an OSGI environment (see STANBOL-811:       * the previously used {@link File#createTempFile(String, String)} does not      * work on some Windows versions.      */
specifier|private
name|File
name|embeddedSolrServerDir
decl_stmt|;
comment|/**      * Embedded Solr server used for {@link #performCVFold(File, int, int, int, boolean)}      * if no ManagedSolrServer is present (e.g. when running outside of OSGI).      * Lazily initialised relative to {@link #embeddedSolrServerDir} on the first      * call to performCVFold      */
specifier|private
name|EmbeddedSolrServer
name|__evaluationServer
decl_stmt|;
specifier|private
name|File
name|__evaluationServerDir
decl_stmt|;
name|void
...
argument_list|)
expr_stmt|;
comment|// if training set is not null, track it
if|if
condition|(
name|trainingSetId
operator|!=
...
expr_stmt|;
block|}
comment|// TODO if training set is null, make it possible to programmatically create a SolrTrainingSet
comment|// instance using the same managed solr server and register it under the same name as the engine
comment|// it-self.
block|}
annotation|@
name|Deactivate
...
expr_stmt|;
block|}
comment|//shutdown the evaluation server and delete the data of the evaluation server
if|if
condition|(
name|__evaluationServer
operator|!=
...
name|ORDER
argument_list|,
literal|"The configured EnhancementEngine "
operator|+
literal|"order MUST BE an Intever value!"
argument_list|,
name|e
argument_list|)
throw|;
...
name|language
operator|+
literal|"' of the ContentItem is not configured as "
operator|+
literal|" active for this Engine (active: "
operator|+
name|acceptedLanguageSet
operator|+
literal|")."
...
operator|+
literal|" textual content from ContentPart %s of ContentItem %s!"
argument_list|,
name|contentPart
operator|.
name|getKey
...
name|warn
argument_list|(
literal|"ContentPart {} of ContentItem {} does not contain any "
operator|+
literal|"text to extract topics from"
argument_list|,
name|contentPart
operator|.
name|getKey
...
argument_list|()
decl_stmt|;
comment|// TODO: extract all languages based on some configuration instead of hardcoding English
name|Text
name|label
init|=
name|representation
...
name|format
argument_list|(
literal|"Error deleting concepts from Solr Core '%s'"
argument_list|,
name|solrCoreId
argument_list|)
decl_stmt|;
...
name|ClassifierException
argument_list|(
literal|"conceptId must not be null or empty"
argument_list|)
throw|;
block|}
name|SolrServer
...
name|format
argument_list|(
literal|"Error removing concept '%s' on Solr Core '%s'"
argument_list|,
name|conceptId
argument_list|,
name|solrCoreId
...
return|;
block|}
comment|/**      * @param conceptUri      *            the topic model to update      * @param metadataEntryId      *            of the metadata entry id of the topic      * @param modelEntryId      *            of the model entry id of the topic      * @param impactedTopics      *            the list of impacted topics (e.g. the topic node and direct children)      * @param primaryTopicUri      * @param broaderConcepts      *            the collection of broader to re-add in the broader field      */
specifier|protected
name|void
name|updateTopic
parameter_list|(
...
literal|0
decl_stmt|;
comment|// NOTE: The folder used to create the SolrServer used for CVFold
comment|//       is now created within the #embeddedSolrServerDir
try|try
block|{
name|evaluationRunning
operator|=
...
condition|)
block|{
comment|// OSGi setup: the evaluation server will be generated automatically using the
comment|// managedSolrServer
name|classifier
operator|.
...
literal|"-evaluation"
argument_list|,
comment|//TODO: maybe we should use the SolrCoreName instead
name|solrCoreConfig
argument_list|)
argument_list|)
expr_stmt|;
...
throw|;
block|}
comment|// clean all previous concepts from the evaluation classifier in case we are reusing an existing solr
comment|// index from OSGi.
name|classifier
operator|.
name|removeAllConcepts
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// bind our new classifier to the same training set at the parent
name|classifier
operator|.
name|setTrainingSet
argument_list|(
...
condition|)
block|{
comment|// we need a minimum about of examples otherwise it's really not
comment|// worth computing statistics
name|skipTopic
operator|=
literal|true
expr_stmt|;
...
name|debug
argument_list|(
literal|"Skipping evaluation of {} because too few positive examples."
argument_list|,
name|topic
argument_list|)
expr_stmt|;
...
name|format
argument_list|(
literal|"Finished CV iteration %d/%d on classifier %s in %fs."
argument_list|,
name|cvFoldIndex
operator|+
literal|1
...
name|format
argument_list|(
literal|"Performance for concept '%s': precision=%f, recall=%f,"
operator|+
literal|" positiveSupport=%d, negativeSupport=%d"
argument_list|,
name|conceptId
argument_list|,
name|precision
...
argument_list|()
decl_stmt|;
comment|// TODO: use OWL property inference on sub-properties here instead of explicit
comment|// property filter
name|Iterator
argument_list|<
name|GraphNode
argument_list|>
