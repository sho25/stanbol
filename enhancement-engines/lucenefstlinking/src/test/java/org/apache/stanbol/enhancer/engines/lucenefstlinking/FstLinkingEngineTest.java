...
decl_stmt|;
comment|/**      * The maximal time we wait for the creation of an FST model in the test      */
specifier|public
specifier|static
specifier|final
name|long
name|FST_CREATION_WAIT_TIME
init|=
literal|2000
decl_stmt|;
comment|//seconds
...
index|[]
block|{
literal|"en;field=dbpedia-ont:surfaceForm;generate=true"
block|}
argument_list|)
decl_stmt|;
name|fstConfig
...
argument_list|()
expr_stmt|;
comment|//now create the FST models
name|List
argument_list|<
name|Future
argument_list|<
...
name|assertTrue
argument_list|(
literal|"Failure in UnitTest - all FST models need to be generate=true"
argument_list|,
name|corpus
operator|.
name|allowCreation
...
block|}
block|}
comment|//and wait until all models are built (should only take some seconds on
comment|//typical hardware
for|for
control|(
name|Future
argument_list|<
...
parameter_list|)
block|{
comment|// we assert on future.isDone instead
block|}
name|Assert
operator|.
name|assertTrue
argument_list|(
literal|"FST Model creation not finished after "
operator|+
name|FST_CREATION_WAIT_TIME
operator|+
literal|"seconds"
...
parameter_list|)
block|{
comment|//create clones from the parsed sets so that we can remove values
name|Set
argument_list|<
name|String
argument_list|>
...
argument_list|)
decl_stmt|;
comment|//iterate over all fise:TextAnnotations
comment|//NOTE this assumes all textAnnotations are from the FST linking engine
name|log
operator|.
name|info
argument_list|(
literal|"  ... validated fise:TextAnnotations:"
argument_list|)
expr_stmt|;
name|Map
argument_list|<
...
argument_list|()
decl_stmt|;
comment|//validate this test annotation against the Stanbol EnhancementStructure
name|EnhancementStructureHelper
operator|.
...
argument_list|)
expr_stmt|;
comment|//NOTE also check for contains in the parsed set to not fail if the
comment|//     same selected text is contained multiple times
name|Assert
operator|.
name|assertTrue
argument_list|(
literal|"fise:selected-text '"
operator|+
name|selectedText
operator|+
literal|"' not expected (expected: "
operator|+
name|expectedSelectedTexts
operator|+
literal|")"
...
operator|++
expr_stmt|;
comment|//count the number of fise:TextAnnotations
block|}
name|Assert
operator|.
name|assertTrue
argument_list|(
literal|"Results do miss following expected fise:TextAnnotations: "
operator|+
name|selectedTexts
argument_list|,
name|selectedTexts
...
name|info
argument_list|(
literal|"  ... validated fise:EntityAnnotations:"
argument_list|)
expr_stmt|;
name|Iterator
argument_list|<
...
comment|//                expectedEntities.contains(entityUri.getUnicodeString()));
name|num
index|[
literal|1
index|]
...
name|assertTrue
argument_list|(
literal|"Results do miss following expected fise:EntityAnnotations: "
operator|+
name|suggestedEntities
argument_list|,
name|suggestedEntities
