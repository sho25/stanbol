...
name|elConfig
decl_stmt|;
comment|/**      * Used in the {@link LinkingModeEnum#NER} to filter entities. For that configured      * mappings for the {@link NerTag#getType()} and {@link NerTag#getTag()} values       * (the key) are mapped with the actual {@link Match#getTypes()} (the value set).       * The<code>null</code> value is interpreted as wildCard (any type matches). An      * empty mapping is interpreted as an blacklist (do not lookup Named Entities      * with that {@link NerTag#getType() type}/{@link NerTag#getTag() tag}      */
specifier|protected
specifier|final
name|Map
argument_list|<
...
name|IllegalArgumentException
argument_list|(
literal|"The NamedEntity type mappings MUST NOT be NULL "
operator|+
literal|"if the LinkingMode is NER!"
argument_list|)
throw|;
block|}
name|this
...
name|warn
argument_list|(
literal|"Unable to process {} with engine name={} and mode={} "
operator|+
literal|": Missing AnalyzedText content part. Please ensure that "
operator|+
literal|"NLP processing results are available before FST linking!"
argument_list|,
operator|new
name|Object
index|[]
...
literal|"Unable to process {} with engine name={} and mode={} "
operator|+
literal|"as the AnalyzedText does not contain any Tokens!"
argument_list|,
operator|new
name|Object
index|[]
...
condition|)
block|{
comment|//require AnalysedText contentPart
name|at
operator|=
name|getAnalysedText
argument_list|(
...
block|{
comment|//AnalysedText is optional in LinkingModeEnum.BASIC
try|try
block|{
name|at
operator|=
...
parameter_list|)
block|{
comment|//unexpected contentPart found under the URI expecting the AnalysedText
name|at
operator|=
literal|null
expr_stmt|;
...
condition|)
block|{
comment|//we can get the content from the Analyzed text
name|content
operator|=
name|at
operator|.
...
block|{
comment|//no analyzed text ... read is from the text/plain blob
try|try
block|{
name|content
operator|=
...
name|ci
argument_list|,
literal|"Unable to access plain/text content!"
argument_list|,
name|e
argument_list|)
throw|;
...
name|warn
argument_list|(
literal|" - missing NE types for Named Entity [{},{}] {}!"
argument_list|,
operator|new
name|Object
index|[]
...
return|;
block|}
comment|/**      * Filter Entities based on matching the entity types with the named entity types.      * The {@link #neTypeMappings} are used to convert named entity types to       * entity types.       * @param eTypes the types of the entity      * @param neTypes the types of the named entity      * @return      */
specifier|private
name|boolean
name|filterByNamedEntityType
parameter_list|(
...
parameter_list|)
block|{
comment|//first collect the allowed entity types
name|Set
argument_list|<
name|String
argument_list|>
...
condition|)
block|{
comment|//found an wildcard
return|return
literal|false
return|;
comment|//do not filter
block|}
else|else
block|{
name|entityTypes
...
block|}
block|}
comment|//else no mapping for neType (tag or uri) present
block|}
block|}
if|if
condition|(
...
literal|true
return|;
comment|//no match possible .. filter
block|}
comment|//second check the actual entity types against the allowed
while|while
condition|(
name|eTypes
operator|.
...
literal|false
return|;
comment|//we found an match .. do not filter
block|}
block|}
comment|//no match found ... filter
return|return
literal|true
return|;
block|}
...
name|debug
argument_list|(
literal|" ... set up TokenStream and TagClusterReducer for linking mode {}"
argument_list|,
name|linkingMode
argument_list|)
expr_stmt|;
...
name|PLAIN
case|:
comment|//will link all tokens and search longest dominant right
name|tokenStream
operator|=
name|baseTokenStream
expr_stmt|;
...
name|NER
case|:
comment|//this uses the NamedEntityTokenFilter as tokenStream and a
comment|//combination with the longest dominant right as reducer
...
name|LINKABLE_TOKEN
case|:
comment|//this uses the LinkableTokenFilter as tokenStream
...
argument_list|)
decl_stmt|;
comment|//NOTE that the  LinkableTokenFilter implements longest dominant right
comment|// based on the matchable span of tags (instead of the whole span).
name|reducer
operator|=
operator|new
name|ChainedTagClusterReducer
...
argument_list|(
literal|"Unrecognized LinkingMode '"
operator|+
name|linkingMode
operator|+
literal|"! Please adapt implementation to changed Enumeration!"
argument_list|)
throw|;
block|}
name|log
...
argument_list|)
expr_stmt|;
comment|//Now process the document
specifier|final
name|long
index|[]
name|time
