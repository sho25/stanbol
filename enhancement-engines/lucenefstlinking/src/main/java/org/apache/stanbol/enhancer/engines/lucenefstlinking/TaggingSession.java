...
name|rankingField
decl_stmt|;
comment|/**      * Used in the {@link LinkingModeEnum#NER} to store the {@link NerTag#getTag()}      * and {@link NerTag#getType()} values for the span of the Named Entity.<p>      * This information is collected by the {@link NamedEntityTokenFilter} while      * iterating over the parsed text and is used in the processing of      * {@link Tag}s to filter Entities based on their types.<p>      * Not used in any linking mode other than<code>NER</code>      */
specifier|protected
specifier|final
name|NavigableMap
argument_list|<
...
name|fieldLoader
decl_stmt|;
comment|/**      * The current version of the SolIndex (as reported by       * {@link DirectoryReader#getVersion()}) of the       * {@link IndexConfiguration#getIndex()}      */
specifier|private
specifier|final
name|Long
name|indexVersion
...
name|config
expr_stmt|;
comment|//init the SolrIndexSearcher
name|searcherRef
operator|=
name|config
operator|.
...
argument_list|)
expr_stmt|;
comment|//check if the IndexConfiguration is up to date with the version of the index
name|long
name|confVersion
init|=
name|config
...
name|debug
argument_list|(
literal|"> update IndexConfiguration (from: {} | to: {}"
argument_list|,
name|confVersion
argument_list|,
name|indexVersion
...
return|;
block|}
comment|/**      * Obtains the FST corpus for the parsed CorpusInfo. The other parameters      * are just used for error messages in case this is not successful.      * @param indexVersion the current version of the index      * @param fstInfo the info about the corpus      * @return the TaggerFstCorpus      * @throws CorpusException if the requested corpus is currently not available      */
specifier|private
name|TaggerFstCorpus
name|obtainFstCorpus
parameter_list|(
...
name|CorpusException
argument_list|(
literal|"Unable to abtain Fst Corpus for "
operator|+
name|fstInfo
operator|+
literal|"(message: "
...
block|{
comment|//fstCorpus != null
comment|//check if the current FST corpus is up to date with the Solr index
if|if
condition|(
name|indexVersion
operator|!=
...
name|debug
argument_list|(
literal|" - FST corpus for language '{}' is outdated"
argument_list|,
name|fstInfo
operator|.
name|language
...
name|warn
argument_list|(
literal|"Unable to update outdated FST corpus for language '{}' "
operator|+
literal|"because runtimeCreation is {} and ExecutorServic "
operator|+
...
name|warn
argument_list|(
literal|"  ... please adapt the Engine configuration for up "
operator|+
literal|"to date FST corpora!"
argument_list|)
expr_stmt|;
block|}
block|}
...
block|{
comment|//FST corpus is up to date with the current Solr index version
name|log
operator|.
name|debug
argument_list|(
literal|"FST corpus for language '{}' is up to date"
argument_list|,
name|fstInfo
operator|.
name|language
...
block|}
block|}
comment|//TODO: maybe make this configurable
name|int
name|waitTime
init|=
name|fstCorpus
...
condition|)
block|{
comment|//we needed to build a new corpus
try|try
block|{
name|log
operator|.
name|debug
argument_list|(
literal|" - will wait max {}sec for creation of {}"
argument_list|,
name|waitTime
argument_list|,
name|fstInfo
...
argument_list|()
expr_stmt|;
comment|//recover interrupted state
block|}
catch|catch
parameter_list|(
name|ExecutionException
...
name|warn
argument_list|(
literal|"Unable to update outdated FST corpus "
operator|+
name|fstInfo
operator|+
literal|" (message: "
...
name|debug
argument_list|(
literal|"unable to build FST corpus for {} in time ({}sec). Will use "
operator|+
literal|"previouse version "
argument_list|,
name|fstInfo
argument_list|,
name|waitTime
...
name|CorpusException
argument_list|(
literal|"Unable to build Fst Corpus for "
operator|+
name|fstInfo
operator|+
literal|"within "
...
name|debug
argument_list|(
literal|"building of  FST corpus for {} was cancelled. Will use "
operator|+
literal|"previouse version."
argument_list|,
name|fstInfo
...
name|CorpusException
argument_list|(
literal|"Building of FST Corpus "
operator|+
name|fstInfo
operator|+
...
argument_list|()
expr_stmt|;
comment|//check again in write lock
if|if
condition|(
name|enqueuedCorpus
operator|==
...
condition|)
block|{
comment|//enqueue for re-creation
name|enqueuedCorpus
operator|=
name|config
operator|.
...
return|;
block|}
comment|/**      * The current version of the SolrIndex as reported by the {@link IndexReader}      * used by this TaggingSession.      * @return the current version of the SolrIndex.      */
specifier|public
name|Long
name|getIndexVersion
parameter_list|()
