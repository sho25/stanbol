...
init|=
literal|"org.apache.stanbol.enhancer.engines.celi.lemmatizer.languages"
decl_stmt|;
specifier|private
name|LanguageConfiguration
name|languageConfig
...
condition|)
block|{
comment|//TODO: maybe still add them and use the Lemmatizer as Tokenizer
continue|continue;
comment|//ignore terms without readings
block|}
comment|//Add the LexicalEntry as Token to the Text. NOTE that if a
comment|//Token with the same start/end positions already exist this
...
argument_list|)
decl_stmt|;
comment|//Now try to get POS annotations for the Token
for|for
control|(
name|Value
argument_list|<
...
condition|)
block|{
comment|//do not override with lover prob
name|tokenLexCats
operator|.
name|put
argument_list|(
...
argument_list|)
decl_stmt|;
comment|//add the readings (MorphoFeatures)
if|if
condition|(
name|mf
operator|!=
...
condition|)
block|{
comment|//use the POS tags of the morpho analysis and compare it
comment|//with existing POS tags.
name|double
name|posProbability
init|=
operator|-
...
block|}
block|}
comment|//add the morpho features with the posProbabiliy
name|Value
argument_list|<
name|MorphoFeatures
argument_list|>
