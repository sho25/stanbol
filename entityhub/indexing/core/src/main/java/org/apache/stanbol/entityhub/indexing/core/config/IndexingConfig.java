...
specifier|final
name|boolean
name|ALLOW_CLASSPATH
init|=
literal|true
decl_stmt|;
comment|/**      * This stores the context within the classpath to load the       */
specifier|private
specifier|final
name|File
...
return|;
block|}
comment|/**      * Searches for a resource with the parsed name in the parsed directory.      * If it can not be found, than it looks up the file within the classpath      * and if found copies it to the parsed location. In case the parsed      * file refers to a directory, then all the contents of that directory      * are copied.       * @param root the (relative path) to the directory containing the file.      * typically on of {@link #configDir} or {@link #sourceDir}.      * @param fileName the name of the file (file or directory)      * @return the absolute File or<code>null</code> if not found.      */
specifier|private
name|File
name|getResource
parameter_list|(
...
name|info
argument_list|(
literal|"> rquested Resource missing "
argument_list|)
expr_stmt|;
name|log
operator|.
name|info
argument_list|(
literal|"     ... try to init via Classpath {}"
argument_list|,
name|classPathRootDir
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"> initialisation of resources via classpath not possible "
argument_list|)
expr_stmt|;
block|}
return|return
...
name|warn
argument_list|(
literal|"No '{}' found via classpath. Will try to init via '{}' but that usually does not work."
argument_list|,
name|INDEXING_PROERTIES
argument_list|,
name|IndexingConfig
...
expr_stmt|;
block|}
comment|//and now the folder representing the rootDir (but within the Classpath)
comment|//to validate if this folder is present in the classpath
name|File
name|indexingClasspathRoot
init|=
operator|new
name|File
argument_list|(
...
condition|)
block|{
comment|//but return still the root, because configDir, ... are all relative
comment|//to this root
return|return
name|classpathRoot
return|;
block|}
