...
begin_comment
comment|/**  * Utility that converts {@link FieldQuery} instances to JSON. This is likely to  * be used by different {@link ModelWriter} implementations as the JSON  * serialized FieldQuery needs to be attached to {@link QueryResultList}s  * serialized to different formats.  * @author Rupert Westenthaler  *  */
end_comment
...
argument_list|)
decl_stmt|;
comment|/**      * Converts the parsed {@link FieldQuery} to a JSON formatted String      * @param query the query      * @param ident the ident. Values< 1 will deactivate pretty formatting      * @param nsPrefixService optionally a {@link NamespacePrefixService} to      * support qNames      * @return the serialized field query      * @throws IOException if the parsed query could not be serialized to JSON      */
specifier|public
specifier|static
name|String
name|toJsonString
...
return|;
block|}
comment|/**      * Converts a {@link Constraint} to JSON      *      * @param constraint the {@link Constraint}      * @param nsPrefixService Optionally the service that is used to convert data type      * URIs to '{prefix}:{localname}'      * @return the JSON representation      * @throws JSONException      */
specifier|private
specifier|static
name|JSONObject
name|convertConstraintToJSON
...
block|}
block|}
comment|//finally write the MODE
if|if
condition|(
name|valueConstraint
operator|.
