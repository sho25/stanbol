...
name|append
argument_list|(
literal|"uses an unknown namespace prefix '"
argument_list|)
expr_stmt|;
name|parsingErrorMessages
operator|.
...
name|constraint
operator|=
name|parseSimilarityConstraint
argument_list|(
name|jConstraint
argument_list|,
name|nsPrefixService
...
throw|;
block|}
comment|//finally parse the optional boost
if|if
condition|(
name|jConstraint
operator|.
...
name|StringBuilder
argument_list|(
literal|"The Boost of a Constraint "
operator|+
literal|"MUST BE a double AND>= 0 (parsed: '"
argument_list|)
decl_stmt|;
name|message
operator|.
...
name|IllegalArgumentException
argument_list|(
literal|"SimilarityConstraints MUST define a \"context\": \n "
operator|+
name|jConstraint
operator|.
name|toString
...
argument_list|)
expr_stmt|;
comment|//finally parse the optional termProximity
if|if
condition|(
name|jConstraint
operator|.
...
name|warn
argument_list|(
literal|"Parsed ValueConstraint does define illegal values (values={})!"
argument_list|,
name|value
argument_list|)
expr_stmt|;
...
name|append
argument_list|(
literal|"Parsed ValueConstraint does define illegal values for field 'value'"
operator|+
literal|"(value MUST NOT contain NULL, JSONObject nor JSONArray values)!\n"
argument_list|)
expr_stmt|;
name|message
operator|.
...
name|append
argument_list|(
literal|"Parsed ValueConstraint does define illegal value for field 'value'"
operator|+
literal|"(value MUST NOT be an JSON object. Only values and JSONArray to parse"
operator|+
literal|"multiple values are allowed)!\n"
argument_list|)
expr_stmt|;
name|message
operator|.
...
return|;
block|}
comment|/**      * Parses the {@link MODE} for {@link ValueConstraint}s and       * {@link ReferenceConstraint}s, by evaluating the 'mode' attribute of      * the parsed {@link JSONObject}      * @param jConstraint the JSON formatted constraint      * @return the parsed {@link MODE} or<code>null</code> if the 'mode'      * attribute is not present      * @throws JSONException if the value of the 'mode' is not an element of the      * {@link MODE} enumeration.      */
specifier|private
specifier|static
name|MODE
name|parseConstraintValueMode
...
name|format
argument_list|(
literal|"Parsed ValueConstraint defines an "
operator|+
literal|"unknown MODE %s (supported: %s)!"
argument_list|,
name|jmode
argument_list|,
name|Arrays
...
name|warn
argument_list|(
literal|"Parsed ReferenceConstraint does not define a single valid \"value\"!"
argument_list|)
expr_stmt|;
name|StringBuilder
name|message
...
name|append
argument_list|(
literal|"This means values where only null, empty string or '{prefix}:{localname}' values with unknown {prefix}\n"
argument_list|)
expr_stmt|;
name|message
operator|.
