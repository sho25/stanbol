...
return|;
block|}
comment|/**      * For some {@link SparqlEndpointTypeEnum SPARQL endpoint types} we need to add an additional constraint to      * determine the ranking information based on incoming relations to the Entities.      *<p>      * Currently this done for {@link SparqlEndpointTypeEnum#ARQ ARQ} and {@link SparqlEndpointTypeEnum#LARQ LARQ}.      *       * @param endpointType      *            the endpoint type      * @param queryString      *            the SPARQL query string to add the ranking constraint      * @param rootFieldName      *            the variable name used to select entities      */
specifier|private
specifier|static
name|void
...
block|}
block|}
comment|// else nothing to do add language Filters
if|if
condition|(
name|constraint
operator|.
...
return|;
block|}
comment|/**      * Version of {@link #addGrammarEscapedValue(StringBuilder, String)} that      * returns the escapedValue      * @param value      * @return      * @see #addGrammarEscapedValue(StringBuilder, String)      */
specifier|protected
specifier|static
specifier|final
name|String
...
return|;
block|}
comment|/**      * Encode Strings as specified in the SPARQL specification section      *<a href="http://www.w3.org/TR/rdf-sparql-query/#grammarEscapes">       * A.7 Escape sequences in strings</a>      * @param query      * @param value      */
specifier|private
specifier|static
specifier|final
name|void
...
parameter_list|)
block|{
comment|// adds (?var>/<[=] "valueString"^^xsd:type)
name|queryString
operator|.
name|append
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//append the string representation of the parsed value
if|if
condition|(
name|value
operator|instanceof
...
argument_list|)
expr_stmt|;
comment|//append the xsd:dataType
name|String
name|xsdType
init|=
name|guessXsdType
...
block|}
block|}
comment|/**      * Return the appropriate XSD type for RDF literals based on the parsed Java class.      * This is used for encoding {@link ValueConstraint} and {@link RangeConstraint}s.      * Based on<code>org.apache.marmotta.commons.sesame.model.LiteralCommons(Class<?> javaClass)</code>      * @param javaClass      * @return      */
specifier|public
specifier|static
name|String
name|guessXsdType
...
parameter_list|)
block|{
comment|// if a string is parsed the query expects a plain literal. For xsd:String
comment|// users need to explicitly parse the data type
comment|//        if(String.class.isAssignableFrom(javaClass)) {
comment|//            return NamespaceEnum.xsd + "string";
comment|//        } else
if|if
condition|(
name|Integer
...
literal|null
return|;
comment|//Namespaces.NS_XSD+"string";
block|}
block|}
specifier|public
specifier|static
