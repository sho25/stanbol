...
decl_stmt|;
comment|/**      * {@link IndexConstraintTypeEncoder} used for MLT requests to encode the parsed fields      */
specifier|private
specifier|final
name|List
argument_list|<
...
control|)
block|{
comment|//we need to get the actual fields in the index for the
comment|//logical fields parsed with the constraint
name|IndexDataTypeEnum
name|mapedIndexTypeEnum
init|=
name|IndexDataTypeEnum
operator|.
name|forDataTyoe
...
argument_list|)
expr_stmt|;
comment|//init IndexValues and check for the dataType
if|if
condition|(
name|rangeConstraint
operator|.
...
argument_list|)
expr_stmt|;
comment|//init the Phrase Query based on the ProximityRanking state
if|if
condition|(
name|textConstraint
operator|.
...
block|{
comment|//TODO: maybe make the default configureable for the SolrYard
name|constraintValue
operator|.
name|setProperty
argument_list|(
...
block|}
block|}
comment|/**      * Utility method that copies over the {@link Constraint#getBoost()} value      * to the {@link ConstraintValue}      */
specifier|private
name|void
name|addBoost
parameter_list|(
...
return|;
block|}
comment|/**          * Sets a property          * @param key the key           * @param value the value          * @return the old value or<code>null</code> if none.          */
specifier|public
name|Object
name|setProperty
parameter_list|(
